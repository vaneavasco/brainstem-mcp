...
- **Decided with the owner: the floor is Node 24**, the version we develop, test and ship in Docker. Nothing older is tested, so nothing older is promised. The manifest states `compatibility.runtimes.node: ">=24"` and a minimum Claude Desktop version (the one that first ships Node 24: to be read from the colleague's installation), so an older app refuses the bundle with a clear message instead of failing at start.
...
- Lifecycle: stdin closing, SIGTERM and SIGINT close the runtime (watcher, reconcile timer) and exit 0, as do a failing stdout (`EPIPE`: the client was killed) and a fatal transport error, so no server outlives its client; an unusable vault path exits 1 with one line on stderr before any protocol byte.
