...
- [x] (a) **Done 2026-08-30** — Claude Code 2.1.251 connected via a live quick-tunnel URL: `claude mcp add --transport http brainstem https://<x>.trycloudflare.com/mcp` → `claude mcp login brainstem` (CIMD `https://claude.ai/oauth/claude-code-client-metadata`, loopback redirect on an ephemeral port) → consent page approved with the owner secret (browser step driven with curl; Chrome extension unavailable in the session) → "Authenticated with brainstem" → `claude mcp get brainstem` shows ✔ Connected → a `claude -p` session wrote `acceptance/hello-from-claude-code.md` through `vault_write` and it appeared in the vault; `_brainstem/state.json` holds one access + one refresh token for client "Claude Code", `connection.md` was written. Still pending: the same from claude.ai web + phone (needs a human browser), tunnel restart ⇒ reconnect, cloudflare-token mode, Windows.
- [x] (b) **Done 2026-08-30 (claude.ai web)** — custom connector added by URL in claude.ai (auth detected as "Always required", client registration detected as CIMD via `https://claude.ai/oauth/mcp-oauth-client-metadata`); consent approved; `oauth token issued` for client "Claude"; the connector showed "Connected" with the 21 tools and their titles; a chat then exercised **19/19 callable tools** (ping, list, write/read/append/edit with dry run, batch read, search, search_frontmatter, batch_frontmatter_update, move, daily note path/read/append, canvas add_node/add_edge/read, analytics summary/findings, soft delete, list) with 0 failures — artifacts verified on disk. Two real-browser bugs were found and fixed during this run: `Referrer-Policy: no-referrer` made the browser send `Origin: null` on the consent POST (`7233b11`), and `form-action 'self'` made Chrome block the post-Approve redirect to claude.ai (`37e905a`). Observations from the run — fixed in this follow-up: (A) a note created via `vault_write` could lose its trailing newline by the time `vault_edit` ran — root cause was `vault_append` (`src/storage/local-fs.ts`), not `vault_write` itself: it inserted a separator newline before appended text but never guaranteed one after it, so a note that started clean (`vault_write` … `vault_append`) could end up non-newline-terminated by the time a later `vault_edit` dry-run diffed it and printed "\ No newline at end of file"; `vault_append` now always leaves the file newline-terminated. (B) the daily note was created without frontmatter when no template is configured (tool text said "from the configured template") — added `DEFAULT_DAILY_TEMPLATE` (frontmatter with `type: daily`/`date:` plus a title) used whenever `DAILY_NOTES_TEMPLATE` is unset. (C) added the missing single-file `vault_frontmatter_update` alongside `vault_batch_frontmatter_update` (21 vault tools total). A new end-to-end contract test, `tests/tools/acceptance-scenario.test.ts`, replays this whole acceptance sequence (ping → list → write/read/append/edit dry-run+apply → write → batch read → search → search_frontmatter → frontmatter_update → batch_frontmatter_update → move → daily note path/read/append → canvas add_node×2/add_edge/read → analytics summary/findings → delete → list) against a real MCP client/server harness, asserting on disk as well as on structured results, so these three regress loudly if reintroduced. Phone: still pending (same hosted callback, same CSP exception).
