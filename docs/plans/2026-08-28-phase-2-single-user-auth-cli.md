...
- [x] (b) **Done 2026-08-30 (claude.ai web)** — custom connector added by URL in claude.ai (auth detected as "Always required", client registration detected as CIMD via `https://claude.ai/oauth/mcp-oauth-client-metadata`); consent approved; `oauth token issued` for client "Claude"; the connector showed "Connected" with the 21 tools and their titles; a chat then exercised **19/19 callable tools** (ping, list, write/read/append/edit with dry run, batch read, search, search_frontmatter, batch_frontmatter_update, move, daily note path/read/append, canvas add_node/add_edge/read, analytics summary/findings, soft delete, list) with 0 failures — artifacts verified on disk. Two real-browser bugs were found and fixed during this run: `Referrer-Policy: no-referrer` made the browser send `Origin: null` on the consent POST (`7233b11`), and `form-action 'self'` made Chrome block the post-Approve redirect to claude.ai (`37e905a`). Observations from the run (fix-later): `vault_write` drops the trailing newline; the daily note is created without frontmatter when no template is configured (tool text says "from the configured template"); no single-file `vault_frontmatter_update` alias (batch only). Phone: still pending (same hosted callback, same CSP exception).
