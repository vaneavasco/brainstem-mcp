...
**Status:** v1.1 — scope frozen 2026-08-27; protocol/auth/infra sections revised the same day after the 2026-readiness review (`docs/reviews/2026-08-27-plan-review.md`)
**Owner:** Vanea
...
| Target | Value | Why |
|---|---|---|
| MCP spec | **2026-07-28** (stateless core, MRTR, header routing, cacheable lists) | Current spec. Serve **legacy 2025-06-18 / 2025-11-25 clients too** via SDK legacy mode — Claude's 2026-07-28 rollout is still in progress as of 2026-08-27. Flip to `legacy: 'reject'` only after verifying claude.ai web + mobile + Desktop + Claude Code all negotiate 2026-07-28. |
...
2. **CIMD (primary):** `client_id` is an HTTPS URL with a path. AS behavior: fetch the document (timeout 5 s, max 64 KB, `Accept: application/json`, **SSRF guard**: resolve DNS, reject loopback/private/link-local/metadata ranges, reject non-HTTPS, do not follow cross-host redirects, pin the resolved IP for the connection); validate JSON, required fields `client_id`, `client_name`, `redirect_uris`; `client_id` in document must equal the URL byte-for-byte; cache per HTTP cache headers (default 1 h, negative-cache 5 min); `redirect_uri` in the request must exactly match an entry — **except loopback (`http://localhost/...`, `http://127.0.0.1/...`) where the port is ignored** (RFC 8252 §7.3; Claude Code needs this). Optional `CIMD_ALLOWED_HOSTS` trust policy (default: allow all, log hostname).
...
- **Consent screen — mandatory, no cookie shortcut** (confused-deputy rule for OAuth proxies with a static upstream client): shows `client_name`, the redirect **hostname** (extra warning if loopback-only), and "this client will read and write your SecondBrain vault". Approve ⇒ continue; Deny ⇒ redirect with `error=access_denied` + `state` + `iss`.
...
- Google code flow via `google-auth-library` `OAuth2Client`: scopes `openid email https://www.googleapis.com/auth/drive.file`, `access_type=offline`, `include_granted_scopes=true`, `state=<pending.id>`, redirect `PUBLIC_URL/oauth/google/callback`. Use **`prompt=consent` only when we have no stored refresh token for the user** (Google caps refresh tokens at 50 per user per client and each consent mints a new one); otherwise `prompt=select_account`. If Google returns no refresh token and we have none stored, re-run with `prompt=consent`.
...
- Google access tokens refreshed on demand (in-memory per-tenant, ~55 min); on `invalid_grant` (revoked / 6-month inactivity) mark `users.reauth_required = true` and return a tool error instructing the user to reconnect the connector. Plane A tokens for that user keep working so the error is visible.
- Google Cloud consent screen in **Production** with only non-sensitive scopes ⇒ no verification audit, no 7-day refresh-token expiry. Brand verification (logo) optional.

### Data model (Postgres)
`users(id, google_sub UNIQUE, email, reauth_required bool, created_at)` · `google_tokens(user_id PK, refresh_token_enc, updated_at)` · `vaults(user_id PK, backend, root_ref, settings jsonb)` · `oauth_clients(client_id PK, kind 'prereg'|'dcr'|'cimd', client_name, redirect_uris jsonb, metadata jsonb, fetched_at, expires_at)` · `oauth_pending(id PK, client_id, redirect_uri, code_challenge, resource, scope, state, created_at, consented_at)` · `oauth_codes(code_hash PK, pending_id, user_id, expires_at, used_at)` · `oauth_tokens(token_hash PK, kind 'access'|'refresh', family_id, user_id, client_id, resource, scope, expires_at, rotated_at, revoked_at)` · `audit_events(id, user_id, ts, op, path, path_to, bytes_before, bytes_after, sha_before, sha_after, request_id, client_name, status, error)`.
...
3. **OAuth AS:** PKCE S256 required and advertised; code single-use with expiry ≤ 60 s; redirect URI exact-match (loopback port-agnostic only); `iss` on every authorization response; refresh rotation with family revocation on reuse; DCR rate-limited; all metadata/issuer pinned to `PUBLIC_URL`; consent screen cannot be bypassed.
...
5. **CIMD SSRF suite:** private/loopback/link-local/metadata IPs, DNS-rebinding (re-resolve), redirects, oversized/slow documents, `client_id` mismatch, non-JSON — all rejected.
...
**Phase 2 — Auth (3.5d):** Plane A RS (PRM, 401 challenge, bearer verifier, audience), Plane A AS (metadata, CIMD fetcher + SSRF guard, DCR fallback, consent page, authorize, token with rotation, `iss`), Plane B (Google IdP, encrypted token storage, reauth handling), tenancy context/registry/rate limit, audit log.
*Accept:* full connect flow completes from **claude.ai web, Claude mobile, and Claude Code** against the Heroku deployment (Claude Code exercises CIMD + loopback; hosted surfaces exercise CIMD-or-DCR); `oauth-as`, `oauth-rs`, `cimd-ssrf`, `transport`, `tenant-isolation` suites green; refresh rotation observed in logs after 1 h.
...
**Phase 4 — Hardening + onboarding (1.5d):** limits, error taxonomy, result-size windowing, logging (pino, no PII/secrets), README + 5-line user onboarding doc (connector URL + 3 example prompts), seed templates written to `00-inbox/README.md` on vault creation, §8 checklist walked item by item.
...
| Var | Required | Notes |
|---|---|---|
| `PUBLIC_URL` | yes | e.g. `https://brainstem-mcp.herokuapp.com`; pins issuer, PRM `resource` (`PUBLIC_URL/mcp`), all advertised endpoints, `iss`. Never derive URLs from `Host`/`X-Forwarded-*`. |
...
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` | yes | OAuth client (Web application type) |
...
Google Cloud setup (manual, before Phase 2): create project, enable Drive API, OAuth consent screen in **Production** with only `openid`, `email`, `drive.file` scopes, authorized redirect `PUBLIC_URL/oauth/google/callback`.
...
2. **Claude's 2026-07-28 support is still rolling out** — we cannot yet verify the modern era end-to-end against claude.ai; legacy mode makes this a non-blocker. Re-test when Anthropic announces GA.
3. **CIMD selection by Claude is behavioral** (needs both metadata flags) — verify on real claude.ai in Phase 2; DCR fallback stays on until then.
