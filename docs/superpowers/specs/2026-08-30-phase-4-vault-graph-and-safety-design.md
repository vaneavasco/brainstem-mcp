...
5. Delete the journal on success. A journal left behind (crash mid-transaction) is detected at boot: the app logs it and leaves it in place for the owner (`connection.md` mentions it); no automatic replay — the owner has the pre-images.
...
- Regex search is ripgrep-only (no JS regex over untrusted patterns), pattern length capped, `--pcre2` never enabled.
