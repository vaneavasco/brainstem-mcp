...
# tool list, write → read → search → links → query → file on disk, connection note.
set -euo pipefail
BASE="${BASE_URL:-http://localhost:3000}"
VAULT="${VAULT_PATH:-./vault-dev}"

echo "[0/8] seed bearer token into $VAULT/_brainstem/state.json"
TOKEN=$(node -e '
...
echo "[1/8] health"; curl -fsS "$BASE/health" | grep -q '"status":"ok"'

echo "[2/8] unauthenticated POST /mcp is rejected with 401"
...
echo "[3/8] tools/list has 30 vault tools"
...
[ "$COUNT" = "30" ] || { echo "expected 30 vault tools, got $COUNT"; exit 1; }

echo "[4/8] vault_write then vault_read"
...
echo "[5/8] vault_search finds the note"
"${INSPECTOR[@]}" --method tools/call --tool-name vault_search --tool-arg query=Smoke | grep -q '00-inbox/smoke.md'

echo "[6/8] vault_links on the smoke note"
"${INSPECTOR[@]}" --method tools/call --tool-name vault_links --tool-arg path=00-inbox/smoke.md | grep -q '00-inbox/smoke.md'

echo "[7/8] vault_query finds the smoke note"
"${INSPECTOR[@]}" --method tools/call --tool-name vault_query --tool-arg pathPrefix=00-inbox | grep -q '00-inbox/smoke.md'

echo "[8/8] file landed in $VAULT, connection note was written"
