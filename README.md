...
- Everything that is *this process's own working state* — the `vault_transaction` journal, and the index cache described below — lives in a **machine-local folder**, never inside the vault: `$XDG_STATE_HOME/brainstem/<hash>` or `~/.local/state/brainstem/<hash>` on Linux, `~/Library/Application Support/brainstem/<hash>` on macOS, `%LOCALAPPDATA%\brainstem\State\<hash>` on Windows — `<hash>` is the first 16 hex characters of the SHA-256 of the vault's real (symlink-resolved) path, so two spellings of the same vault share one folder. A vault may live in git or in a folder synced between machines (iCloud Drive, Dropbox, Syncthing), where an in-flight transaction journal or a machine-bound cache would be the wrong thing to sync, upload or merge — this keeps it out. Override the base directory with `BRAINSTEM_STATE_HOME=<absolute path>` in `.env` or the environment; `STATE_DIR` (test/dev-only, see `.env.example`) still overrides the journal's folder directly, same as before this split. **Neither override may resolve inside the vault (or the vault inside it)**: a `BRAINSTEM_STATE_HOME` that would land there is refused at boot (exit 1, one stderr line) before anything is created — tools would otherwise list, read and search the server's own working files; the one exception is `STATE_DIR` set to exactly `<vault>/_brainstem` (the reserved folder, today's dev/test override), which is vault content anyway. If the machine-local state folder already holds a `vault.json` naming a *different* vault than the one now being served (only reachable by hand-pointing one of these overrides at a folder that already belongs to another vault), that's logged as one warning line at boot and the file is left exactly as it was — never silently overwritten. Nothing here bends the vault's own `_brainstem/` reservation: every tool still refuses to list, read, write or search it.

**Several stdio sessions on one vault are normal** — every Claude Code session, and every Claude Desktop restart, starts its own. Each one registers itself at `<local folder>/instances/<pid>.json` (written atomically, and re-written every 60 s so a live, merely-idle process never looks stale) and prunes any entry whose pid is no longer alive, or whose file has gone quiet for more than three heartbeats (180 s) — the portable defence against a crashed server's pid being picked up by an unrelated process later. When another session is already running, the new one logs one line on stderr saying how many, and `brainstem_ping`'s `localPeers` field (stdio only, absent on the HTTP server) shows the live count at any moment. What actually protects a vault written by more than one process at once is unchanged: `expectedHash` turns a colliding write into a `CONFLICT` instead of a lost write, and each process keeps its own in-memory index fresh through its filesystem watcher and periodic reconcile. **One vault per server** is still the design — a Desktop install serves one vault, and a second vault in Claude Code is a second `claude mcp add` entry with its own name (above), not a second vault served by one process.

### The index cache

A fresh index build reads every note; on a 37,000-note vault that's about 30 seconds, and the stdio server starts a new process per session. So on the second (and every later) start, it loads a **machine-local cache** of the frontmatter index instead of reading the whole vault again — measured on a 40,000-note vault: a cold deferred boot to "index ready" took about 19.7 s (including a roughly 0.4 s cache save at the end), a warm one about 3.5 s.

**The cache is a hint, never a source.** Every entry in it is used only when the file it names has, in *this* boot's own directory listing, exactly the same size and modification time the cache recorded; anything else — changed, new, or simply not in the cache — is read from disk exactly as it would be without one. Saving is deliberately conservative: an entry whose frontmatter holds a number JSON cannot round-trip (`NaN`, `±Infinity`, `-0`, anywhere, even nested) is never written, so a cold and a warm answer can never disagree over one; an entry whose `modifiedAt` is not comfortably older than the save itself (git's own "racily clean" rule, adapted: a file can be rewritten — same size, same coarse mtime — in the instant around a save) is left out too, and is simply read from disk again next boot. The one known blind spot: a file rewritten with the same size and the same modification time *well before* the save that cached it (a coarse filesystem clock, or a tool that deliberately restores both, e.g. `cp -p`/`touch -d`) is served stale until the next background reconcile pass notices something else about it changed, or until it changes size or mtime. This is the same blind spot `FrontmatterIndex.reconcile` already has for its own drift detection — the cache does not attempt to close it.

It lives next to the state folder above but is a **separate** machine-local folder, under a separate env var: `$XDG_CACHE_HOME/brainstem/<hash>` or `~/.cache/brainstem/<hash>` on Linux, `~/Library/Caches/brainstem/<hash>` on macOS, `%LOCALAPPDATA%\brainstem\Cache\<hash>` on Windows (`<hash>` is the same vault key as above), overridable with `BRAINSTEM_CACHE_HOME=<absolute path>`. Like `BRAINSTEM_STATE_HOME`, this may never resolve inside the vault (or the vault inside it) — unlike the state folder that's never fatal: it just means running without a cache, logged as a single warning line, same as any other cache folder that cannot be created or written. It holds one file, `index-v<N>.ndjson` (`N` bumps whenever the index's own shape changes, so an old cache is never read as if it were a new one), one JSON entry per line — read with a splitter that only ever breaks a line on a raw newline byte (never `readline`, which also breaks a line on U+2028/U+2029, characters a note's own path or frontmatter can legitimately contain) and enforces a 4 MiB per-line cap on the way in and the way out, dropping an oversized line's bytes as they arrive rather than buffering them. The whole file is entirely safe to delete at any time — the next boot just does a cold fill and rewrites it. A leftover tmp file from a save that never finished is removed once its writer's pid is no longer alive (any age) or, for a still-alive writer, once it's a day old. `BRAINSTEM_INDEX_CACHE=off` in `.env` or the environment disables it outright, for the same cold-every-time behaviour as before this feature existed. It is saved once after the index becomes ready (only when it was absent, invalid, or more than 1% of notes had to be re-read from disk), at most once an hour while a session keeps running with changes since the last save, and once more on a clean shutdown if anything changed — a shutdown triggered by the client simply vanishing (a broken pipe, not an orderly disconnect) skips that last save for a very large index, so a departed client's session never spends its shutdown window on a write nobody is waiting for. `brainstem_ping`'s `index.cache` field (stdio only) reports this boot's own numbers: `used`, `entriesFromCache`, `entriesRead`, `skipped` (cache lines dropped while loading, for any reason), and `rejected` (the reason, if a cache was found but thrown out whole — a schema or vault mismatch, a corrupt header). Only the stdio server uses this cache; the HTTP server's blocking boot has no need of it today.
