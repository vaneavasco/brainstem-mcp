...
- **A note deleted, moved or rewritten by a tool can no longer be overwritten in the index by
  the file watcher.** On macOS a rename reaches the watcher as a change; its refresh had read the note
  before the move and wrote it back after the tool's own removal. A refresh that began before
  such a removal or write now discards its result. (Found by the macOS leg of the suite: the
  first as a resurrected deleted note, the second as a rewritten note keeping its old links.)
...
- `vault_search_frontmatter` takes `pathPrefix` (hits under one folder, at a folder boundary, as
  in `vault_query`) and returns `total`. A reader who wanted one year of a field got 417 of 678
  vault-wide hits, 42,000 characters, before switching tools.
